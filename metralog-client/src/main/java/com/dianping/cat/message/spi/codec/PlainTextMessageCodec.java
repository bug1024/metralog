...
public class PlainTextMessageCodec implements MessageCodec {
    ...
    private static final String VERSION2 = "PT2"; // plain text version 2
    private static final String VERSION3 = "PT3"; // plain text version 3
    ...
    private void decodeHeaderOld(Context ctx, MessageTree tree) {
    ...
    protected void decodeHeader(Context ctx, MessageTree tree) {
        ...
        if (VERSION.equals(id)) {
            String sessionToken = helper.read(ctx, LF);
            tree.setSessionToken(sessionToken);
        } else if (VERSION2.equals(id) || VERSION3.equals(id)) {
            String sessionToken = helper.read(ctx, TAB);
            tree.setSessionToken(sessionToken);

            String source = helper.read(ctx, LF);
            // tree.setSource(id);
        } else {
            String warning = String.format("Unrecognized id(%s) for plain text message codec!", id);
